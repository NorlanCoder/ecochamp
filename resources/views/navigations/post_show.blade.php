        ...
        <div class="lg:w-[400px] w-full bg-white h-full relative overflow-y-auto shadow-xl dark:bg-dark2 flex flex-col justify-between" id="post">
            ...
            <div class="p-5 pb-0">
                ...
                <div class="flex gap-3 text-sm font-medium">
                    <img src="{{ empty($item->user->profile) ? asset('/images/avatars/avatar.png') : asset(Storage::url($item->user->profile)) }}" alt="" class="w-9 h-9 rounded-full">
                    <div class="flex-1">
                    ...
                    <div class="flex items-center gap-4 text-xs font-semibold">
                            ...
                            <form action="{{ route('liked_post.store') }}" method="POST" enctype="multipart/form-data" id="form-data-post{{ $item->id }}">
                                ...
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <a href="#" id="liked-post">
                                        {{ count($item->postLikeds) >= 100 ? number_format(count($item->postLikeds) / 100, 1) . 'k' : count($item->postLikeds) }}
                                    </a>
                                </div>
                            </form>
                        </div>
                        <div class="flex items-center gap-3">
                            ...
                            <span id="countComment_post{{ $item->id }}">
                                {{ !empty($item->comments) ? count($item->comments) : 0 }}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative text-sm font-medium space-y-5" id="addComment_post{{ $item->id }}">
                    @foreach($item->comments as $comment)
                        <div class="flex items-start gap-3 relative">
                            <img src="{{ empty($comment->user->profile) ? asset('/images/avatars/avatar.png') : asset(Storage::url($comment->user->profile)) }}" alt="" class="w-6 h-6 mt-1 rounded-full">
                            <div class="flex-1">
            ...
            <form action="{{ route('comment.store') }}" method="POST" enctype="multipart/form-data" id="form-data-post-comment{{ $item->id }}">
                ...
                <div class="bg-white p-3 text-sm font-medium flex items-center gap-2">
                    <img src="{{ empty($item->user->profile) ? asset('/images/avatars/avatar.png') : asset(Storage::url($item->user->profile)) }}" alt="" class="w-6 h-6 rounded-full">
