            ...
            <div class="flex-1">
                ...
                <div class="box overflow-hidden">
                    ...
                    <div class="relative h-80">
                        ...
                        <img src="{{ asset(Storage::url($alert->image_principale)) }}" class="h-36 mb-4 w-full h-full object-cover" alt="Image principale">
                        ...
                        <!-- <div class="p-6 w-full z-10 absolute bg-gradient-to-t bottom-0 from-black/60 hidden">
                            ...
                            <div class="flex items-center gap-5 mt-4 !text-white">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="font-medium text-xs">{{ $alert->created_at->format('j F Y H:i') }}</div>
                                </div>
                                <div class="text-sm ml-auto text-gray-400">{{ $alert->alert_type }}</div> 
                                <div class="text-sm ml-auto text-gray-400">
                                    {{ \Carbon\Carbon::parse($alert->date_remarque)->format('j F Y H:i') }}
                                </div>
                            </div>
                    ...
                    <div class="p-6">
                        <div class="sm:p-4 p-2.5 flex items-center gap-4 text-xs font-semibold">
                            ...
                            <button type="button" uk-toggle="target: #share-ecochamp" class="button-icon ml-auto">
                        ...
                        <div class="flex gap-3 text-sm mt-6">
                            @if(empty($alert->user->profile))
                                <img src="{{ asset('/images/avatars/avatar.png') }}" class="w-9 h-9 rounded-full" alt="Avatar"> 
                            @else
                                <img src="{{ asset(Storage::url($alert->user->profile)) }}" class="w-9 h-9 rounded-full" alt="Avatar"> 
                            @endif
                            <div class="flex-1">
                                <h4 class="text-black font-medium dark:text-white">{{ $alert->user->firstname }} {{ $alert->user->lastname }}</h4>
                                <div class="text-gray-500 font-medium text-xs dark:text-white/80">{{ $alert->created_at->format('j F Y H:i') }}</div>
                            </div>
                            ...
                            <div class="flex items-start gap-3 relative">
                                <a href="{{ url('/profile', $comment->user->id) }}"> 
                                    @if(empty($comment->user->profile))
                                        <img src="{{ asset('/images/avatars/avatar.png') }}" alt="Avatar" class="w-6 h-6 mt-1 rounded-full"> 
                                    @else
                                        <img src="{{ asset(Storage::url($comment->user->profile)) }}" alt="Avatar" class="w-6 h-6 mt-1 rounded-full"> 
                                    @endif
                                </a>
                                <div class="flex-1">
                    ...
                    <form action="{{ route('alert_comment.store') }}" method="POST" enctype="multipart/form-data" id="form-data-alert-comment{{ $alert->id }}">
                        ...
                        <div class="sm:px-4 sm:py-3 p-2.5 border-t border-gray-100 flex items-center gap-1 dark:border-slate-700/40">
                            @if(empty($user->profile))
                                <img src="{{ asset('/images/avatars/avatar.png') }}" alt="Avatar" class="w-6 h-6 rounded-full"> 
                            @else
                                <img src="{{ asset(Storage::url($user->profile)) }}" alt="Avatar" class="w-6 h-6 rounded-full"> 
                            @endif
                            ...
                            <div class="flex-1 relative overflow-hidden h-10">
                                ...
                                <input id="alert_id" type="hidden" name="alert_id" value="{{ $alert->id }}" />
                            </div>

                            <button type="button" class="text-sm rounded-full py-1.5 px-3.5 bg-secondary submit-form" id="create_new">Envoyer</button>
                        </div> 
                    </form>
                                ...
                                <div class="flex items-center gap-3">
                                    <a href="{{ url('/profile', $item->user->id) }}">
                                        @if(empty($item->user->profile))
                                            <img src="{{ asset('/images/avatars/avatar.png') }}" alt="Avatar" class="bg-gray-200 rounded-full w-10 h-10"> 
                                        @else
                                            <img src="{{ asset(Storage::url($item->user->profile)) }}" alt="Avatar" class="bg-gray-200 rounded-full w-10 h-10"> 
                                        @endif
                                    </a>
                                    <div class="flex-1">
                                        <a href="{{ url('/profile', $item->user->id) }}">
                                            <h4 class="font-semibold text-sm text-black dark:text-white">{{ $item->user->lastname }} {{ $item->user->firstname }}</h4>
                                        </a>
                                        <div class="mt-0.5">Vu {{ $item->created_at->format('l j F Y') }}</div>
                                    </div>
                                </div>
                            @endforeach
